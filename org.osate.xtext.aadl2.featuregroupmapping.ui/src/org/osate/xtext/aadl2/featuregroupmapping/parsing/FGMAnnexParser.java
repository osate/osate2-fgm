...
public class FGMAnnexParser implements org.osate.annexsupport.AnnexParser { 
	// Need to get aadl resources for resolving components
	// For resolving package names etc
	
	
    private Injector injector = OsateCorePlugin.getDefault().getInjector("org.osate.xtext.aadl2.featuregroupmapping.FeatureGroupMapping");
	...
	protected  FeatureGroupMappingParser getParser() {
			if (fgmParser == null) {
				...
				fgmParser = injector.getInstance(FeatureGroupMappingParser.class);
			}
		return fgmParser;
	}
	...
	public AnnexLibrary parseAnnexLibrary
		...
		AnnexLibrary eal = (AnnexLibrary) parse(source,getGrammarAccess().getFGMLibraryRule(),filename,line,column, errReporter);
		 return eal;
	...
	public AnnexSubclause parseAnnexSubclause
		...
		AnnexSubclause eas = (AnnexSubclause) parse(source,getGrammarAccess().getFGMSubclauseRule(),filename,line,column, errReporter);
		 return eas;
	 }
